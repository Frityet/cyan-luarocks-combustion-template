...
local prefix        = os.getenv("PREFIX")       or (osname == "Darwin" and "/usr/local" or "/usr/")
local libzip_basedir= os.getenv("LIBZIP_DIR")   or prefix
local lua_basedir   = os.getenv("LUA_DIR")      or prefix

local use_luajit = not not os.getenv("USE_LUAJIT")

print("\x1b[31mMAKE SURE YOU CONFIGURE `combust-config.lua`!\x1b[0m")
